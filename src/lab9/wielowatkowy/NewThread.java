...
public class NewThread implements Runnable {
    ...
    public String login(String log) {
        ...
        if (splitedArray.length == 2) {
            if (splitedArray[0].equals(name) && splitedArray[1].equals(haslo)) {
                ...
                return "Zalogowałeś się, Twoje ID to:" + id;
            } else {
                return "Bledne haslo, odleglość Levenhsteina to :" + Integer.toString(levenshtein(haslo, splitedArray[1]));
            }
        } else {
    ...
    public String find(String login) throws IOException {
        FileReader fileReader = new FileReader("C:/Users/Adrian/Desktop/pliki/serwer/haslo.txt");
        BufferedReader bufferedReader = new BufferedReader(fileReader);
        String textLine = bufferedReader.readLine();
        do {
            String tmp=login(login+";"+textLine);
            String []tmp1=tmp.split(" ");
            if (tmp1[0].equals("Zalogowałeś")) {

                return tmp;
            }
            textLine = bufferedReader.readLine();
        } while (textLine != null);
        return "Zły login lub brak hasla w pliku";
    }
    ...
    public void run() {
        ...
        try {
            ...
            while ((inputLine = in.readLine()) != null) {
                ...
                } else if (tmp.equals("FIND") && splitedArray.length == 2) {
                        out.println(find(splitedArray[1]));
                } else {
